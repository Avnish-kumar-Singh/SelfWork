...
public class deleteproduct extends HttpServlet {
    ...
    protected void doDelete(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            PreparedStatement ps = con.prepareStatement("DELETE FROM semm WHERE id = ?");
            ps.setString(1, id);

            int count = ps.executeUpdate();

            if (count > 0) {
                out.print("<h3>Product deleted Successfully... Plz check your database it was deleted or not..</h3>");
                RequestDispatcher rd = request.getRequestDispatcher("./deleteproduct.jsp");
                rd.include(request, response);
            } else {
                out.print("<h3>Error: Product id miss-matched, Which you want to delete...</h3>");
